...
class FreeModel(FSModel):
    ...
    def metrics(self):
        ...
        """
        metrics = []
        # Check for compilation - compatible with newer Keras versions
        is_compiled = hasattr(self, '_jit_compile') and self._jit_compile
        if not is_compiled and hasattr(self, 'compiled_loss'):
            is_compiled = self.compiled_loss is not None
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape(persistent=False) as tape:
            ...
            # Trainable variables are automatically watched by GradientTape
        ...
        # The custom_metrics section below handles cymetric-specific metrics properly
        if self.custom_metrics is not None:
            loss_dict = {}
